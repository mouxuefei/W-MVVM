...
import com.util.ktx.BuildConfig
import java.security.MessageDigest

class MGlideRoundedCornersTransform(context: Context, radius: Float, private val mCornerType: CornerType) : CenterCrop() {
    private val mRadius: Float = context.dp2px(radius.toInt()).toFloat()
    ...
    override fun updateDiskCacheKey(messageDigest: MessageDigest) {
        messageDigest.run { update(ID_BYTES) }
    }

    companion object {
        private val VERSION = 1
        private val ID = BuildConfig.VERSION_NAME + "MGlideRoundedCornersTransform." + VERSION
